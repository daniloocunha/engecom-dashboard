/**
 * Build Script — Gerador de config.js para Netlify
 *
 * Executa durante o build do Netlify:
 *   node scripts/build-config.js
 *
 * Variáveis de ambiente necessárias no Netlify:
 *   SPREADSHEET_ID   — ID da planilha Google Sheets
 *   SHEETS_API_KEY   — API Key do Google (com restrição HTTP Referrer)
 *   SECRET_KEY       — Senha de acesso ao dashboard (ex: rumo01)
 *   APPS_SCRIPT_URL  — URL do Google Apps Script para escrita (pode ser vazio)
 */
...
// ── Validação das env vars obrigatórias ────────────────────────────────────────
const required = ['SPREADSHEET_ID', 'SHEETS_API_KEY', 'SECRET_KEY'];
const missing  = required.filter(v => !process.env[v]);

if (missing.length > 0) {
    console.error('❌ ERRO: Variáveis de ambiente obrigatórias não definidas:');
    missing.forEach(v => console.error(`   - ${v}`));
    console.error('\nConfigure-as em: Netlify → Site settings → Environment variables');
    process.exit(1);
}
...
// ── Conteúdo gerado do config.js ──────────────────────────────────────────────
const configContent = `/**
 * Configurações do Dashboard de Medição
 * Engecom/Encogel - RUMO Logística
 *
 * ATENÇÃO: Arquivo auto-gerado pelo build do Netlify.
 * NÃO editar diretamente. NÃO commitar no Git.
...
const METAS = {
    ...
    // TSs (Turmas de Solda)
    HH_POR_SOLDADOR_DIA: 8,         // Horas por soldador por dia
    SOLDADORES_TS: 1,                // Número padrão de soldadores TS
    META_DIARIA_TS: 8,               // 1 × 8 = 8 HH/dia (apenas soldador)
...
function getCampoNormalizado(obj, fieldName, defaultValue = '') {
    ...
    // Tentar variações conhecidas primeiro
    if (variacoes[fieldName]) {
        for (const variacao of variacoes[fieldName]) {
...
// ── Gravar arquivo ─────────────────────────────────────────────────────────────
const outputPath = path.join(__dirname, '..', 'dashboard', 'js', 'config.js');

try {
    fs.writeFileSync(outputPath, configContent, 'utf8');
    console.log('✅ config.js gerado com sucesso em:', outputPath);
    console.log(`   SPREADSHEET_ID : ${SPREADSHEET_ID}`);
    console.log(`   SECRET_KEY     : ${SECRET_KEY}`);
    console.log(`   APPS_SCRIPT_URL: ${APPS_SCRIPT_URL || '(não definida)'}`);
} catch (err) {
    console.error('❌ Erro ao gravar config.js:', err.message);
    process.exit(1);
}
