...
class CalendarioTP {
    ...
    _calcularHHMerged(hisArray, operadoresDefault = 12) {
        ...
        // Pré-filtro: justificativas neutras (almoço/DDS/trânsito) e eventos abaixo da
        // duração mínima do catálogo (ex.: trem < 20 min) são descartados antes do merge
        const filtered = intervals.filter(iv => {
            if (!JustificativasHI.considerarHI(iv.tipo)) return false;
            if ((iv.endMin - iv.startMin) < JustificativasHI.minutosMinimos(iv.tipo)) return false;
            return true;
        });
        ...
        rdosDia.forEach(rdoDia => {
            ...
            const servicosRDO = this.dados.servicos
                ...
                .map(s => ({
                    ...
                    coeficiente:   parseFloat(s.Coeficiente || s.coeficiente || 0),
    ...
    renderizarFaixaDias(turmas) {
        ...
        // Cabeçalho: inicial do dia da semana + número do dia embaixo, em cada coluna
                ...
                <div class="modal-dialog modal-xl">
                    <div class="modal-content">
                        ...
                        <div class="modal-body">
                            ...
                            <!-- Serviços -->
                            <div class="mb-4">
                                <h6 class="text-muted mb-3"><i class="fas fa-tools me-2"></i>Serviços Realizados</h6>
                                ${dados.servicos.length === 0 ? '<p class="text-muted mb-2 view-only"><i class="fas fa-info-circle me-2"></i>Nenhum serviço registrado</p>' : `
                                <div class="table-responsive">
                                    <table class="table table-sm table-hover">
                                        ...
                                        <tbody id="tbody-servicos">
                                            ${dados.servicos.map((s, i) => `<tr id="srv-row-${i}">${editorRDO._htmlSrvRow(i, s)}</tr>`).join('')}
                                        </tbody>
                                    </table>
                                </div>
                                `}
    ...
    renderizarTodos() {
        ...
        const tpsVisiveis = tps.filter(tp =>
            !this.turmaFiltro || this.turmaFiltro === 'todas' || this.turmaFiltro === tp
        );

        let html = this.renderizarFaixaDias(tpsVisiveis);
        tpsVisiveis.forEach(tp => {
